...
import { clubsNavigate } from './actions';
...
class Clubs extends Component {
  static propTypes = {
    goToQR: PropTypes.func.isRequired,
    goToClub: PropTypes.func.isRequired,
  };
  ...
  renderClub = (club) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => this.props.goToClub({ item: club })}
      >
  ...
  renderActivity = activity => (
    ...
    >
      <Text style={{ fontSize: 24, fontFamily: 'sports-48-x-48' }}>{sport[activity.className] || ''}</Text>
    </View>
  );
      ...
      <View style={{ flex: 1, flexDirection: 'column' }}>
        <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
          {loading ? (
            ...
            <ScrollView>{clubs.map(club => this.renderClub(club))}</ScrollView>
          )}
        </View>
        <QrButton onPress={() => this.props.goToQR({ type: 'scan', mode: '1' })} />
      </View>
    );
  }
}
...
const mapDispatchToProps = dispatch => ({
  goToQR: params => dispatch(clubsNavigate('QrCode', params)),
  goToClub: params => dispatch(clubsNavigate('Club', params)),
});
